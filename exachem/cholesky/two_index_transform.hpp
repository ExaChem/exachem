...
public:
  // Constructor
  TwoIndexTransform() = default;

  // Destructor
  virtual ~TwoIndexTransform() = default;

  TwoIndexTransform(const TwoIndexTransform&)                = default;
  TwoIndexTransform(TwoIndexTransform&&) noexcept            = default;
  TwoIndexTransform& operator=(const TwoIndexTransform&)     = default;
  TwoIndexTransform& operator=(TwoIndexTransform&&) noexcept = default;

  virtual void print_mo_vectors_analysis(ChemEnv& chem_env, Matrix& C_alpha_eig, Matrix& C_beta_eig,
    ...
    if(vec_analysis.first) {
      ...
      if(is_rhf)
        ...
        cout << endl
             << std::string(10, ' ') << "UHF Final Alpha Molecular Orbital Analysis" << endl;
      ...
      for(auto i = 0; i < sys_data.nbf; i++) {
        ...
        for(size_t j = 0; j < (size_t) sys_data.nbf_orig; j++) {
          if(std::abs(C_alpha_eig(j, i)) >= vec_analysis.second) {
            ...
            cout << mksp << std::setw(4) << std::right << j + 1 << mksp << std::setw(10)
                 << std::right << C_alpha_eig(j, i) << mksp << std::setw(3) << std::right
                 << atom_pos + 1 << "  " << std::left << std::setw(2)
                 << bsm.atominfo[atom_pos].symbol << "  " << std::left << std::setw(2)
                 << bsm.bf_comp[j] << endl;
          }
        }
      }
  ...
  virtual void two_index_transform(ExecutionContext& ec, ChemEnv& chem_env) {
    // libint2::BasisSet& shells    = chem_env.shells;
    Tensor<T> C_alpha_AO = chem_env.scf_context.C_AO;
    Tensor<T> C_beta_AO  = chem_env.scf_context.C_beta_AO;
    Tensor<T> F_alpha_AO = chem_env.scf_context.F_AO;
    Tensor<T> F_beta_AO  = chem_env.scf_context.F_beta_AO;
    Tensor<T> F_MO       = chem_env.cd_context.d_f1;
    ...
    if(rank == 0) {
      ...
      if(is_mso) {
        ...
        print_mo_vectors_analysis(chem_env, C_alpha_eig, C_beta_eig, F_MO_alpha, F_MO_beta);

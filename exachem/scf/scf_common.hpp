...
inline BasisSetMap construct_basisset_maps(std::vector<libint2::Atom>& atoms,
...
inline void compute_orthogonalizer(ExecutionContext& ec, SystemData& sys_data, SCFVars& scf_vars,
...
inline void scf_restart_test(const ExecutionContext& ec, const SystemData& sys_data,
...
inline void scf_restart(const ExecutionContext& ec, const SystemData& sys_data,
                        const std::string& filename, EigenTensors& etensors,
...
inline std::tuple<size_t, double, double>
gensqrtinv(ExecutionContext& ec, SystemData& sys_data, SCFVars& scf_vars,
           ScalapackInfo& scalapack_info, TAMMTensors& ttensors, bool symmetric, double threshold) {
  using T = TensorType;
...
inline std::tuple<shellpair_list_t, shellpair_data_t>
compute_shellpairs(const libint2::BasisSet& bs1, const libint2::BasisSet& _bs2,
...
inline Matrix compute_shellblock_norm(const libint2::BasisSet& obs, const Matrix& A) {
...
inline GauXC::Molecule make_gauxc_molecule(const std::vector<libint2::Atom>& atoms) {
  GauXC::Molecule mol;
...
inline GauXC::BasisSet<double> make_gauxc_basis(const libint2::BasisSet& basis) {

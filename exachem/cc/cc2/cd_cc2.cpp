...
void cd_cc2_driver(ExecutionContext& ec, ChemEnv& chem_env) {
  ...
  if(is_rhf)
    std::tie(residual, corr_energy) = cc2_cs::cd_cc2_cs_driver<T>(
    ...
    std::tie(residual, corr_energy) = cc2_os::cd_cc2_os_driver<T>(
      chem_env, ec, MO, CI, d_t1, d_t2, d_f1, d_r1, d_r2, d_r1s, d_r2s, d_t1s, d_t2s, p_evl_sorted,
  ...
  if(rank == 0) {
    if(is_rhf)
      std::cout << std::endl
                << "Time taken for Closed Shell CC2: " << std::fixed << std::setprecision(2)
                << cc2_time << " secs" << std::endl;
    else
      std::cout << std::endl
                << "Time taken for Open Shell CC2: " << std::fixed << std::setprecision(2)
                << cc2_time << " secs" << std::endl;
  }

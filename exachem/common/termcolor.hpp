...
inline HANDLE get_terminal_handle(std::ostream& stream) {
  ...
  else if(&stream == &std::cerr || &stream == &std::clog) return GetStdHandle(STD_ERROR_HANDLE);
  return nullptr;
}
...
inline HANDLE get_terminal_handle(std::wostream& stream) {
  ...
  else if(&stream == &std::wcerr || &stream == &std::wclog) return GetStdHandle(STD_ERROR_HANDLE);
  return nullptr;
}

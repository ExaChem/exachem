...
public:
  ...
  // Destructor
  virtual ~EOMGuessOpt() = default;

  EOMGuessOpt(const EOMGuessOpt&)            = default;
  EOMGuessOpt& operator=(const EOMGuessOpt&) = default;
  EOMGuessOpt(EOMGuessOpt&&)                 = default;
  EOMGuessOpt& operator=(EOMGuessOpt&&)      = default;

  // Main member function
  virtual void eom_guess_opt(ExecutionContext& ec, const TiledIndexSpace& MO,
                             TiledIndexSpace& hbar_tis, int& nroots, const TAMM_SIZE n_occ_alpha,
                             const TAMM_SIZE n_occ_beta, std::vector<T>& p_evl_sorted,
                             std::vector<Tensor<T>>& x1, bool left = false) {
    // Scheduler sch{ec};
...
void eom_guess_opt(ExecutionContext& ec, const TiledIndexSpace& MO, TiledIndexSpace& hbar_tis,
  ...
  EOMGuessOpt<T> eom_guess;
  eom_guess.eom_guess_opt(ec, MO, hbar_tis, nroots, n_occ_alpha, n_occ_beta, p_evl_sorted, x1,
                          left);
}

...
public:
  ...
  bool assign(Float d) {
    ...
    else biased_e = 1; // Subnormals use biased exponent 1 (min exponent).
...
inline fp get_cached_power(int min_exponent, int& pow10_exponent) {
  ...
  int index = static_cast<int>(((min_exponent + fp::significand_size - 1) * (significand >> shift) +
...
FMT_INLINE digits::result grisu_gen_digits(fp value, uint64_t error, int& exp, Handler& handler) {
  ...
  do {
    ...
    result         = handler.on_digit(static_cast<char>('0' + digit), power_of_10_64(exp) << -one.e,
...
struct cache_accessor<float> {
  ...
  static carrier_uint
  compute_right_endpoint_for_shorter_interval_case(const cache_entry_type& cache,
                                                   int beta_minus_1) FMT_NOEXCEPT {
    return static_cast<carrier_uint>(
...
struct cache_accessor<double> {
  ...
  static carrier_uint compute_round_up_for_shorter_interval_case(const cache_entry_type& cache,
                                                                 int beta_minus_1) FMT_NOEXCEPT {
    return ((cache.high() >> (64 - float_info<double>::significand_bits - 2 - beta_minus_1)) + 1) /
           2;
  }
};
...
FMT_INLINE decimal_fp<T> shorter_interval_case(int exponent) FMT_NOEXCEPT {
  ...
  else if(ret_value.significand < xi) { ++ret_value.significand; }
  return ret_value;
}
...
small_divisor_case_label:
  ...
  if((dist & mask) == 0) {
    ...
    else { ret_value.significand += dist; }
  }
...
void fallback_format(Double d, int num_digits, bool binary32, buffer<char>& buf, int& exp10) {
  ...
  const bool is_predecessor_closer = binary32 ? value.assign(static_cast<float>(d))
                                              : value.assign(d);

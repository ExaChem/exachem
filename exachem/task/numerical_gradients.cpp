...
void compute_gradients(ExecutionContext& ec, ChemEnv& chem_env, std::vector<Atom>& atoms,
  ...
  if(ec.print()) {
    std::cout << std::endl << chem_env.task_string << " Energy Gradients" << std::endl;
    for(size_t i = 0; i < chem_env.ec_atoms.size(); i++) {
      std::cout << std::setw(4) << std::left << chem_env.ec_atoms[i].esymbol << std::right
                << std::setw(16) << std::fixed << std::setprecision(10) << gradients(i, 0)
                << std::right << std::setw(16) << std::fixed << std::setprecision(10)
                << gradients(i, 1) << std::right << std::setw(16) << std::fixed
                << std::setprecision(10) << gradients(i, 2) << std::endl;
    }
  }
}

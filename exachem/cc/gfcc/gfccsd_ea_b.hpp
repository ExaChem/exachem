...
template<typename T>
class GFCCSD_EA_B_Driver {
public:
  // Constructor
  GFCCSD_EA_B_Driver() = default;

  // Destructor
  virtual ~GFCCSD_EA_B_Driver() = default;

  // Copy constructor
  GFCCSD_EA_B_Driver(const GFCCSD_EA_B_Driver&)            = default;
  GFCCSD_EA_B_Driver& operator=(const GFCCSD_EA_B_Driver&) = default;
  GFCCSD_EA_B_Driver(GFCCSD_EA_B_Driver&&)                 = default;
  GFCCSD_EA_B_Driver& operator=(GFCCSD_EA_B_Driver&&)      = default;

  /**
   * @brief Driver function for electron affinity GF-CCSD (beta spin)
   */
  virtual void gfccsd_driver_ea_b(

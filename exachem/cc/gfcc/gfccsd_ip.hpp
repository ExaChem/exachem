...
public:
  ...
  // Destructor
  virtual ~GFCCSD_IP_A() = default;

  // Copy constructor and assignment
  GFCCSD_IP_A(const GFCCSD_IP_A&)            = default;
  GFCCSD_IP_A& operator=(const GFCCSD_IP_A&) = default;
  GFCCSD_IP_A(GFCCSD_IP_A&&)                 = default;
  GFCCSD_IP_A& operator=(GFCCSD_IP_A&&)      = default;

  virtual void gfccsd_x1_a(Scheduler& sch, const TiledIndexSpace& MO, Tensor<std::complex<T>>& i0_a,
                           ...
                           const Tensor<std::complex<T>>& x2_aaa,
                           const Tensor<std::complex<T>>& x2_bab, const Tensor<T>& f1,
                           const Tensor<T>& ix2_2_a, const Tensor<T>& ix1_1_1_a,
                           const Tensor<T>& ix1_1_1_b, const Tensor<T>& ix2_6_3_aaaa,
                           const Tensor<T>& ix2_6_3_abab, const TiledIndexSpace& gf_tis,
                           bool has_tis, bool debug = false);

  virtual void gfccsd_x2_a(
    ...
    const Tensor<std::complex<T>>& x2_bab, const Tensor<T>& f1, const Tensor<T>& ix2_1_aaaa,
    const Tensor<T>& ix2_1_abab, const Tensor<T>& ix2_2_a, const Tensor<T>& ix2_2_b,
    const Tensor<T>& ix2_3_a, const Tensor<T>& ix2_3_b, const Tensor<T>& ix2_4_aaaa,
    const Tensor<T>& ix2_4_abab, const Tensor<T>& ix2_5_aaaa, const Tensor<T>& ix2_5_abba,
    const Tensor<T>& ix2_5_abab, const Tensor<T>& ix2_5_bbbb, const Tensor<T>& ix2_5_baab,
    const Tensor<T>& ix2_6_2_a, const Tensor<T>& ix2_6_2_b, const Tensor<T>& ix2_6_3_aaaa,
    const Tensor<T>& ix2_6_3_abba, const Tensor<T>& ix2_6_3_abab, const Tensor<T>& ix2_6_3_bbbb,
    const Tensor<T>& ix2_6_3_baab, const Tensor<T>& v2ijab_aaaa, const Tensor<T>& v2ijab_abab,
    const Tensor<T>& v2ijab_bbbb, const TiledIndexSpace& gf_tis, bool has_tis, bool debug = false) {
    ...
    const auto [p3_va, p4_va, p5_va, p8_va, p9_va]                = v_alpha.labels<5>("all");
    const auto [p3_vb, p4_vb, p5_vb, p8_vb, p9_vb]                = v_beta.labels<5>("all");
    const auto [h1_oa, h2_oa, h6_oa, h7_oa, h8_oa, h9_oa, h10_oa] = o_alpha.labels<7>("all");
    const auto [h1_ob, h2_ob, h6_ob, h7_ob, h8_ob, h9_ob, h10_ob] = o_beta.labels<7>("all");
    ...
    //    ( i0_temp(p4,h1,h2)             =         x2(p8,h1,h7)  * ix2_5(h7,p4,h2,p8)       )
    //    //O3V2 ( i0(p4,h1,h2)                 += -1    * i0_temp(p4,h1,h2) ) ( i0(p4,h2,h1) +=
    //    i0_temp(p4,h1,h2)                        ) (   i_6(h10,h1,h2)              = -1    *
    //    x1(h8)        * ix2_6_1(h8,h10,h1,h2)    ) (   i_6(h10,h1,h2)             += x2(p5,h1,h2)
    //    * ix2_6_2(h10,p5)          ) (   i_6_temp(h10,h2,h1)         =         x2(p9,h1,h8)  *
    //    ix2_6_3(h8,h10,h2,p9)    ) //O4V (   i_6(h10,h1,h2)             += -1    *
    //    i_6_temp(h10,h1,h2)                      ) (   i_6(h10,h2,h1)             +=
    //    i_6_temp(h10,h1,h2)                      ) ( i0(p3,h1,h2)                  += d_t1(p3,h10)
    //    * i_6(h10,h1,h2)       )

    //    (   i_10(p5)                    =  0.5  * x2(p8,h6,h7)  * v2(h6,h7,p5,p8)          )
    //    ( i0(p3,h1,h2)                 +=  1.0  * d_t2(p3,p5,h1,h2)  * i_10(p5)              )

    //    (   i_11(h6,h1,p5)              =         x2(p8,h1,h7)  * v2(h6,h7,p5,p8)          )
    //    //O3V2 ( i0_temp(p3,h2,h1)             =         d_t2(p3,p5,h1,h6)  * i_11(h6,h2,p5) )
    //    //O3V2 ( i0(p3,h1,h2)                 +=         i0_temp(p3,h1,h2) ) ( i0(p3,h2,h1) += -1
    //    * i0_temp(p3,h1,h2)                        )
...
public:
  ...
  // Destructor
  virtual ~GFCCSD_IP_B() = default;

  // Copy constructor and assignment
  GFCCSD_IP_B(const GFCCSD_IP_B&)            = default;
  GFCCSD_IP_B& operator=(const GFCCSD_IP_B&) = default;
  GFCCSD_IP_B(GFCCSD_IP_B&&)                 = default;
  GFCCSD_IP_B& operator=(GFCCSD_IP_B&&)      = default;

  virtual void gfccsd_x1_b(Scheduler& sch, const TiledIndexSpace& MO, Tensor<std::complex<T>>& i0_b,
                           const Tensor<T>& t1_a, const Tensor<T>& t1_b, const Tensor<T>& t2_aaaa,
                           const Tensor<T>& t2_bbbb, const Tensor<T>& t2_abab,
                           const Tensor<std::complex<T>>& x1_b,
                           const Tensor<std::complex<T>>& x2_bbb,
                           const Tensor<std::complex<T>>& x2_aba, const Tensor<T>& f1,
                           const Tensor<T>& ix2_2_b, const Tensor<T>& ix1_1_1_a,
                           const Tensor<T>& ix1_1_1_b, const Tensor<T>& ix2_6_3_bbbb,
                           const Tensor<T>& ix2_6_3_baba, const TiledIndexSpace& gf_tis,
                           bool has_tis, bool debug = false);

  virtual void gfccsd_x2_b(
    ...
    const Tensor<std::complex<T>>& x2_aba, const Tensor<T>& f1, const Tensor<T>& ix2_1_bbbb,
    const Tensor<T>& ix2_1_baba, const Tensor<T>& ix2_2_a, const Tensor<T>& ix2_2_b,
    const Tensor<T>& ix2_3_a, const Tensor<T>& ix2_3_b, const Tensor<T>& ix2_4_bbbb,
    const Tensor<T>& ix2_4_abab, const Tensor<T>& ix2_5_aaaa, const Tensor<T>& ix2_5_abba,
    const Tensor<T>& ix2_5_baba, const Tensor<T>& ix2_5_bbbb, const Tensor<T>& ix2_5_baab,
    const Tensor<T>& ix2_6_2_a, const Tensor<T>& ix2_6_2_b, const Tensor<T>& ix2_6_3_aaaa,
    const Tensor<T>& ix2_6_3_abba, const Tensor<T>& ix2_6_3_baba, const Tensor<T>& ix2_6_3_bbbb,
    const Tensor<T>& ix2_6_3_baab, const Tensor<T>& v2ijab_aaaa, const Tensor<T>& v2ijab_abab,
    const Tensor<T>& v2ijab_bbbb, const TiledIndexSpace& gf_tis, bool has_tis, bool debug = false) {
    using ComplexTensor = Tensor<std::complex<T>>;
    TiledIndexSpace o_alpha, v_alpha, o_beta, v_beta;
...
template<typename T>
void exachem::cc::gfcc::GFCCSD_IP_A<T>::gfccsd_x1_a(
  Scheduler& sch, const TiledIndexSpace& MO, Tensor<std::complex<T>>& i0_a, const Tensor<T>& t1_a,
  const Tensor<T>& t1_b, const Tensor<T>& t2_aaaa, const Tensor<T>& t2_bbbb,
  const Tensor<T>& t2_abab, const Tensor<std::complex<T>>& x1_a,
  const Tensor<std::complex<T>>& x2_aaa, const Tensor<std::complex<T>>& x2_bab, const Tensor<T>& f1,
  const Tensor<T>& ix2_2_a, const Tensor<T>& ix1_1_1_a, const Tensor<T>& ix1_1_1_b,
  const Tensor<T>& ix2_6_3_aaaa, const Tensor<T>& ix2_6_3_abab, const TiledIndexSpace& gf_tis,
  bool has_tis, bool debug) {
  TiledIndexSpace o_alpha, v_alpha, o_beta, v_beta;
...
template<typename T>
void exachem::cc::gfcc::GFCCSD_IP_B<T>::gfccsd_x1_b(
  Scheduler& sch, const TiledIndexSpace& MO, Tensor<std::complex<T>>& i0_b, const Tensor<T>& t1_a,
  const Tensor<T>& t1_b, const Tensor<T>& t2_aaaa, const Tensor<T>& t2_bbbb,
  const Tensor<T>& t2_abab, const Tensor<std::complex<T>>& x1_b,
  const Tensor<std::complex<T>>& x2_bbb, const Tensor<std::complex<T>>& x2_aba, const Tensor<T>& f1,
  const Tensor<T>& ix2_2_b, const Tensor<T>& ix1_1_1_a, const Tensor<T>& ix1_1_1_b,
  const Tensor<T>& ix2_6_3_bbbb, const Tensor<T>& ix2_6_3_baba, const TiledIndexSpace& gf_tis,
  bool has_tis, bool debug) {
  TiledIndexSpace o_alpha, v_alpha, o_beta, v_beta;
  ...
  const auto [p7_va]               = v_alpha.labels<1>("all");
  const auto [p7_vb]               = v_beta.labels<1>("all");
  const auto [h1_oa, h6_oa, h8_oa] = o_alpha.labels<3>("all");
  const auto [h1_ob, h6_ob, h8_ob] = o_beta.labels<3>("all");
  const auto [u1]                  = gf_tis.labels<1>("all");

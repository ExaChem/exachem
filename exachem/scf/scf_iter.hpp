...
void diis(ExecutionContext& ec, const TiledIndexSpace& tAO, Tensor<TensorType> D,
          Tensor<TensorType> F, Tensor<TensorType> err_mat, int iter, int max_hist,
          const SCFVars& scf_vars, const int n_lindep, std::vector<Tensor<TensorType>>& diis_hist,
...
scf_iter_body(ExecutionContext& ec, ScalapackInfo& scalapack_info, const int& iter,
  ...
  if(is_ks) { sch(F_alpha() += VXC()).execute(); }
#endif
  ...
  if(iter >= 1 && !ediis) {
    if(is_rhf) {
      ++scf_vars.idiis;
      diis(ec, tAO, D_alpha_tamm, F_alpha, err_mat_alpha_tamm, iter, max_hist, scf_vars,
    ...
    if(is_uhf) {
      ...
      diis(ec, tAO, D_T, F1_T, err_mat_alpha_tamm, iter, max_hist, scf_vars, sys_data.n_lindep,
           ttensors.diis_hist, ttensors.fock_hist);
      diis(ec, tAO, D_S, F1_S, err_mat_beta_tamm, iter, max_hist, scf_vars, sys_data.n_lindep,
...
void diis(ExecutionContext& ec, const TiledIndexSpace& tAO, Tensor<TensorType> D,
  ...
  if(ndiis > max_hist) {
    ...
    if(!scf_vars.switch_diis) {
      ...
      for(size_t i = 0; i < diis_hist.size(); i++) { max_err[i] = tamm::norm(diis_hist[i]); }
  ...
  sch(F() = 0).execute();
  for(int j = 0; j < idim; j++) { sch(F() += X_final[j] * fock_hist[j]()); }

...
class SCFIter {
protected:
  // Keep helpers protected
  virtual void scf_diis(ExecutionContext& ec, const ChemEnv& chem_env, const TiledIndexSpace& tAO,
  ...
  virtual void compute_2c_ints(ExecutionContext& ec, const ChemEnv& chem_env,
                               EigenTensors& etensors, const SCFData& scf_data,
                               TAMMTensors<T>& ttensors);

  virtual void compute_2bf_ri_direct(ExecutionContext& ec, const ChemEnv& chem_env,
...
public:
  SCFIter()                              = default;
  virtual ~SCFIter()                     = default;
  SCFIter(const SCFIter&)                = default;
  SCFIter& operator=(const SCFIter&)     = default;
  ...
  compute_2bf_taskinfo(ExecutionContext& ec, const ChemEnv& chem_env, const SCFData& scf_data,
                       const bool do_schwarz_screen, const std::vector<size_t>& shell2bf,
